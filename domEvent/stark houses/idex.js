let got = {
  ...
  let rootpeople=document.querySelector('.allpeople')
  let roothouse=document.querySelector('.allhouse')
  let rootinput=document.querySelector('input')
...
function handlesearch(event){
    let searchtext=event.target.value
     let allpeoplename=allpeoples.filter(p=>p.name.toLowerCase().includes(searchtext.toLowerCase()))
     console.log(allpeoplename)
     creatpeople(allpeoplename)
}

rootinput.addEventListener('input',handlesearch)


  function creatpeople(data=[]){
    rootpeople.innerHTML=''
    data.forEach(people=>{
       let li=document.createElement('li')
       li.classList.add('flex-32')
  ...
  function creathouse(data=[]){
    roothouse.innerHTML=''
    data.forEach(name=>{
        let button=document.createElement('a')
        button.innerText=name
        button.classList.add('flex-5')
        if(activehouse==name){
            button.classList.add('active')
        }
        button.addEventListener('click',()=>{
            activehouse=name
            let peoplename=got.houses.find((house)=>house.name===name).people ||[]
            console.log(peoplename)
            creatpeople(peoplename)
            creathouse(allhouses)
        })
        roothouse.append(button)
    })
  }

  creatpeople(allpeoples)
